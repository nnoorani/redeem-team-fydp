        ...
        #starting other camera threads
                while True: 
                        ...
                        if not q.empty():
                                ...
                                threading.Thread(target=capture, args=(cameras[0],curr_timestamp, "cam1")).start()
                                print "starting 1"
                                #time.sleep(3)
                                threading.Thread(target=capture, args=(cameras[2],curr_timestamp, "cam2")).start()
                                print "starting 2"
                                #time.sleep(3)
                                threading.Thread(target=capture, args=(cameras[3],curr_timestamp, "cam3")).start()
                                print "starting 3"
...
def check_if_object_present(im):
        ...
        im2 = im[250:270,580:675] # crop image to around the pixels we want to look at
        path = "pixel"
        ...
        references = [[25,75,95],[0,0,65],[80,30,0]] #this is the colours we EXPECT them to be
